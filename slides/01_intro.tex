...
\begin{frame}
  \frametitle{Glossary}
    ...
    \item[noeud] un ``ordinateur'' indépendant
...
\begin{frame}
  \frametitle{Glossaire --- difficulté avec ``\emph{thread}''}
...
\begin{frame}
  ...
  \begin{exampleblock}{Solution}
    ...
    \item Chaque CPU ``contrôle'' une partie de la mémoire.
      \begin{itemize}
...
\begin{frame}
  ...
  \begin{itemize}
  \item un noeud SMP avec $2 \times$ CPU AMD EYPC ``Naples''
...
\begin{frame}[fragile]
  ...
  \item<2-> ``Annotations'' de parallélisme dans des programmes C (\red{OpenMP}, OpenACC)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
\frametitle{\emph{Strong scaling} (``extensibilité forte'')}
...
(Sur)co\^ut d\^u au parallélisme `` $1-E(n,p)$''~ 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
\frametitle{\emph{Weak scaling} (``extensibilité faible'')}
