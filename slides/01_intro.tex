...
\begin{frame}
  ...
  \begin{columns}[t]
    ...
    \column{0.5\textwidth}
    \begin{center}
      \begin{tikzpicture}[>=stealth,scale=0.5, every node/.style={shape=ellipse, font=\small,inner sep=2pt}]
        \path[use as bounding box] (-3, 0.5) rectangle (7 , -8);
        \node (nature) {Nature};
        \node[below=0.5cm of nature,draw] (obs) {Observations};
        \node[below=0.5cm of obs, draw] (theory) {Theory};
        \node[below left=0.5cm of theory, draw, align=center,inner sep=0] (exp) {Physical\\experiments};
        \node<2>[below right=0.5cm of theory, draw, align=center,red,inner sep=1pt] (sim) {Numerical\\simulation};

        \draw (nature) edge[->] (obs);
        \draw (obs) edge[->] (theory);
        \draw (theory) edge[->] (exp);
        \draw<2> (theory) edge[->] (sim);

        \draw[->] (exp.north)  |- (obs);
        \draw<2>[red,->] (sim.north)  |- (obs);
...
\begin{frame}
  \frametitle{Non-Uniform Memory Access}
  \begin{tikzpicture}
    \node {\includegraphics[width=\textwidth]{dell2}};
    \draw[red,ultra thick,dashed] (-5, 0) -- (5, 0);
  \end{tikzpicture}
\end{frame}
