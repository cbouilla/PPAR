...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Procéder par phases}
\subsection{Sparse LU}

\begin{frame}[label=sparse_lu_intro]
  \frametitle{Example: Sparse LU Factorization}
...
\begin{frame}[label=sparse_lu_example]
  ...
  \end{column}
  \begin{column}{6.5cm}
    \begin{alertblock}<10->{Several columns in parallel?}
      \begin{itemize}
      \item Conflicting accesses to the rows!
      \end{itemize}
    \end{alertblock}
    ...
    \begin{exampleblock}<11->{Solution}
      Identify \emph{independent} column\textbf{S}
    \end{exampleblock}
  \end{column}
\end{columns}
...
\begin{block}{Algorithm}
  ...
  \begin{enumerate}
  \item~[Start.] $j \gets 1$
  \item~[Search.] Find $i$ such that $M_{ij} \neq 0$
  \item~[Eliminate.] For $M_{i'j} \neq 0$ and $i' \neq i$, do: $M_{i'} \gets (...) \times M_i$
  \item~[Loop.] Increment $j$. If $j=n$, STOP. Otherwise go to step 2
  \end{enumerate}
\end{block}
\end{frame}
...
\begin{frame}
  \frametitle{General Idea \#2: Work in ``Phases''}
  ...
  \begin{columns}[c]
    ...
    \begin{column}{.9\textwidth}
      \begin{itemize}
      \item Give up a small amount of parallelism...
      \end{itemize}
    \end{column}
  \end{columns}
  ...
  \begin{columns}[c]
    ...
    \begin{column}{.9\textwidth}
      \begin{itemize}
      \item ... To eliminate conflits altogether
      \end{itemize}
    \end{column}
  \end{columns}  
\end{frame}
...
\end{column}
\begin{column}{0.6\textwidth}
  \begin{alertblock}{Dependency}
    \begin{itemize}
    \item Columns $j$ and $j'$ connected by row $i$
    \end{itemize}
  \end{alertblock}

  \medskip

  \begin{block}{Dependency graph $G_{dep}$}
    \begin{itemize}
    \item Vertices $V = $ all columns
    \item Edges:
      \[
  ...
  \begin{exampleblock}{Independent columns}
    $\rightarrow$ Independent Set in $G_{dep}$
  \end{exampleblock}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}[label=sparse_lu_MIS]
  ...
  \begin{block}{Modified algorithm:}
    \begin{itemize}
    \item While not all columns have been eliminated:
      \begin{itemize}
      \item Find an independent set $\mathcal{I}$ in $G_{dep}$.
        \begin{itemize}
        \item (optimal = NP-hard! Here: sequential greedy algorithm)
        \end{itemize}
      \item Eliminate all columns in $\mathcal{I}$ \alert{in parallel}
      \item ``Forget'' eliminated columns
      \end{itemize}
    \end{itemize}
  \end{block}
...
\begin{frame}[label=sparse_lu_MIS_3]
  \frametitle{Example: Sparse LU Factorization}
  \framesubtitle{Sub-Example: Greedy Algorithm for Maxim\alert{al} Independent Set}
  
  \begin{block}{Algorithm (can be implemented in linear time)}
    \begin{itemize}
    \item $\mathcal{I} \gets \emptyset$
    \item While $G$ is not empty:
      \begin{itemize}
      \item Choose a vertex $x$ (arbitrarily)
      \item Add $x$ to $\mathcal{I}$
      \item Remove $x$ and all its neighbors from $G$
      \end{itemize}
...
\end{tikzpicture}
  ...
  \begin{columns}[c]
    ...
    \begin{column}[c]{.9\textwidth}
      ...
      In parallel: neighborhood conflict...
...
\begin{frame}[label=parallel_MIS]
  ...
  \begin{exampleblock}{Modified Algorithm}
    \begin{itemize}
    \item $\mathcal{I} \gets \emptyset$
    \item Choose a random permutation $\pi$ of $\{1, \dots, n\}$  (``\emph{score}'')
    \item While $G$ is not empty: \uncover<2>{\hfill(\alert{\emph{$\approx \log^2 n$ iterations}})}
      \begin{itemize}
      \item $X = \{ u \in V~|~\text{$\pi[u] > \pi[v]$ for all $u \leftrightarrow v$} \}$ %$
      \item Add $X$ to $\mathcal{I}$.
      \item Remove $X$ and all neighbors of verticies in $X$ from $G$
      \end{itemize}
    \item Return $\mathcal{I}$
    \end{itemize}
  \end{exampleblock}
...
\begin{frame}[label=sparse_lu_MIS]
  ...
  \begin{block}{Alternative algorithm (other point of view):}
  \begin{itemize}
  \item While not all columns have been eliminated:
    \begin{itemize}
    \item Set $\mathcal{I} \gets \empty$, then for each remaining column (in parallel):
      \begin{itemize}
      \item Row indices in the column: $[i_1, \dots, i_k]$
      \item~\emph{(Optional)} If $\texttt{Mark}[i_\ell] = 1$ for any $\ell$, abort
      \item For all $\ell$, do:\\
        $\quad$ $old \gets \texttt{Mark}[i_\ell]$ and $\texttt{Mark}[i_\ell] \gets 1$ (atomic capture).\\
        $\quad$ If $old = 1$, abort.
      \end{itemize}
    \item Eliminate all colonnes in $\mathcal{I}$ \alert{in parallel}
    \item ``Forget'' eliminated columns and reset \texttt{Mark} to zero
    \end{itemize}
  \end{itemize}
\end{block}
...
\bigskip
  ...
  \begin{columns}[c]
    ...
    \begin{column}[c]{.9\textwidth}
      ...
      Some rows spuriously ``locked'' until the next phase 
...
\begin{frame}[label=idea1]
  \frametitle{General Idea \#3: \textbf{Analyze Conflict Frequency}}
